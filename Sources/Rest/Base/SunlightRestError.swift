...
public extension SunlightRestClient {
  struct RestError: Error, CustomStringConvertible, LocalizedError {
    public static var unableToParseResponse: RestError {
      return RestError(status: .unableToParseResponse)
    }

    public static var unableToParseRequest: RestError {
      return RestError(status: .unableToParseRequest)
    }

    public static var unknownError: RestError {
      return RestError(status: .unknown)
    }
    ...
    public init(error: Error) {
      if let RestError = error as? RestError {
        self.status = RestError.status
        self.jsonPayload = RestError.jsonPayload
      } else {
    ...
    // Confirm to CustomStringConvertible protocol
